...
initial_guess = [1.01e6,100,60,-1.25,1];
fit_list_allparams = [];

data_est = fLorentz(ff,initial_guess(1),initial_guess(2),initial_guess(3),initial_guess(4),initial_guess(5));
% figure (1);
% plot (f,real(data_px));
...
[params_estimated,fval] = fminunc( fitFn, initial_guess );
y_estimated = fLorentz( ff, params_estimated(1), params_estimated(2), params_estimated(3), params_estimated(4), params_estimated(5));

f0est = params_estimated(1);
Aest = params_estimated(2);
Qest = params_estimated(3);
Phi0est = params_estimated(4);
PhiLin = params_estimated(5);

figure (7);
plot(f, abs (data_px),'o','markersize',14);
xlabel('Frequencies in Hz');
ylabel('Magnitude arb');
title(sprintf('Magnitude: Fit pixel (%i,%i): f0=%0.2f MHz, A=%0.1f, Q=%0.1f, Phi =%0.1f rad \n PhiLin =%0.2f',pxi,pxj,f0est/1e6,Aest,Qest,Phi0est,PhiLin));
...
xlabel('Frequencies in Hz');
ylabel('Phase rad');
title(sprintf('Phase: Fit pixel (%i,%i): f0=%0.2f MHz, A=%0.1f, Q=%0.1f, Phi =%0.1f rad \n PhiLin =%0.2f',pxi,pxj,f0est/1e6,Aest,Qest,Phi0est,PhiLin));
